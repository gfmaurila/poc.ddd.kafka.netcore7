...
public class CreateUserPerfilKafkaConsumer : BackgroundService
{
    private readonly IKafkaMessageBusService _messageBusService;
    private readonly IServiceProvider _serviceProvider;
    private const string TOPIC_NAME = "CREATE_USER_PERFIL";

    public CreateUserPerfilKafkaConsumer(IServiceProvider serviceProvider, IKafkaMessageBusService messageBusService)
    {
    ...
    public async Task CreateUserRedis(byte[] message)
    {
        ...
        using (var scope = _serviceProvider.CreateScope())
        {
            ...
            await createUserRedis.CreateRedis($"user_perfil_id_{info.Id}", info, 2);
        }
    }
}
